...
// Creating a thread using ThreadStart delegate which does not take any parameters and does not return any value
...
for (int i = 0; i < 10; i++)
{
    // Creating a thread using ParameterizedThreadStart delegate which takes an object parameter and does not return any value
    Thread thread = new Thread(new ParameterizedThreadStart(new DemoPrinter().Print));
    thread.Name = $"Thread-{i}";
    // Pass the values to the thread, when it starts
    thread.Start(i);
}
...
public class DemoPrinter
{
    public void Print()
    {
        ...
        Console.WriteLine($"On threadId : {thread.ManagedThreadId}, threadName : {thread.Name}");
    }
    ...
    public void Print(object? value)
    {
        int i = (int)value;
        var thread = Thread.CurrentThread;
        Console.WriteLine($"Value {i} on threadId : {thread.ManagedThreadId}, threadName : {thread.Name}");
    }
}